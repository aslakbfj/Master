...
from torch.nn.parameter import Parameter # creates custom activations with
...
class ConvNetShallow(nn.Module):
    """
    Shallow CNN model with 1 convolutional layer designed for input
    sequences of length 200 bp
    """
    def __init__(self, num_classes, weight_path=None):
        """
...
class NonStrandSpecific(nn.Module):
    """
    Adapted from Selene: non_strand_specific_module.py

    A `torch.nn.Module` that wraps a user-specified model architecture
    if the architecture does not need to account for sequence
    strand-specificity.
